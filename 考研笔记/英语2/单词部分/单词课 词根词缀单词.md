### 前结

人物观点题：

​	Sb+v+that(从句)		-->某人认为（作者态度肯定）

​	Sb+doubt/deny+that(从句)		-->某人不认为(作者态度否定)

翻译题目：

​	"some reason" some有表示"某种"的意思 译作"某种原因"

<=7原则：

​	当一个单词的字母数量小于等于7时，这个单词一般都同时具有两种词性

词缀词根小结表：(其他未列出的在代码块中会给出)

| 前缀                                       | 词根                      | 后缀                         |
| ------------------------------------------ | ------------------------- | ---------------------------- |
| 前缀表方向                                 | 词根表根本含义            | 后缀表词性                   |
| pro-  (向前)                               | -fess-/-fest-  (speak 说) | -ize  v 动词                 |
| com-/con-/cor-/col-/co-  (come 一起，完全) | -voc-/-vok-  (voice 声音) | -ate  v/n/a 动词居多         |
| a+词根首字母双写 (表加强效果 如：attract)  | -claim-                   | -ion  n 名词                 |
|                                            |                           | -or/-er/-ee/-ent/-ant  n(人) |
|                                            |                           | -al a 形容词                 |

## -fess-/-fest- = speak

### profess

profess	*vt.*自称，（尤指）伪称；公开承认（某种感受或信仰）；信仰，信奉（宗教）；正式准予加入（be professed）

```
profess与confess的差异
同：都有表示承认，坦白的意思
异：情感色彩 profess↑	confess↓
```

profession	*n.*（尤指需要长期训练和正规资格的）职业；（某）职业界同行，同业

```
vocation	n.（认为某种职业适合自己的）使命感；（认为特别适合自己的）工作，职业，生活方式；神召，天命，（宗教）使命感
employment	n.雇用；使用，利用；工作，就业；就业人数
work/job
amateur	n.业余爱好者；生手，外行 adj.非专业的，业余爱好的；不熟练的
```

professional	*adj.*职业的，专业的；非常内行的，极为称职的；（人）职业性的，非业余的；由职业人员参加的；<非正式，贬>（人）坚持某种活动特征（或特点）的

​						n.专业人员，专业人士；内行，专家；职业选手；（受雇于高尔夫或网球协会的）职业教练

professor	*n.*教授



### confess

confess	*v.*供认，招供；承认，坦白；忏悔，告解；聆听忏悔

confesso/er	*n.*忏悔者	也可指神父



### pro-

progress	*n.*进步，进展；前进，行进；（社会的）进步

​					v.进步，进展；使（任务，事业）取得持续进展；（时间）推移，流逝；缓慢行进；（占星）计算行星（或星象图各星及坐标）的位置

process	*n.*步骤，程序；（自然或偶然的）变化过程；（为达到某目标的）过程，进程；制作方法，加工方法；<法律>传票；（生，剖）端突，突起

​				*v.*（用化学物品或机器）处理，加工；审核，受理（正式文件或请求）；（计算机）处理（数据）；冲洗（照片）；加工（食品）；<正式>列队行进；把（头发）弄成直发

project

promote	*v.*促进，提倡；升职，晋升；促销，推广

```
-mote (move 走)
```

prominent	*adj.*重要的，著名的；显眼的，突出的；突起的，高耸的



### con-/com-

compose	*v.*组成，构成；作曲；撰写（信、诗或演讲稿）；使镇静，使平静；为（照片、图像）构图；排版，排稿

```
pose	n.姿势
```

contract	*n.*合同，契约；

```
短语
sign a contract	签合同
traction	n.拖，拉，牵引，拉力，牵引力；
tractor	n.拖拉机，牵引机
attract	(at-tract)v.吸引；（物体间）产生引力；引起；有吸引力
```

consist	*v.*由……组成，由……构成；在于，存在于

```
consist of 由...组成
```

correlate	*v.*相互关联；显示紧密联系

cooperate	*v.*合作，协作；配合，协助

```
operate	v.操纵，操作（机器或设备）；（机器等）运转，运行 
operator	n.（设备、机器的）操作员；经营者；
近义词
collaborate	v.合作，协作；勾结，通敌
	labor	n.劳动；工作；劳工；分娩	vi.劳动；努力；苦干
team work
```

collect	v.聚集，聚积；收集，采集

```
lect (选择，说)
	lecture
a collection of	很多
a succession of 很多
```

combine	v.（使）结合，组合；（使）化合；合并，协力

```
-bine (binary 二)
combination	n.结合体，联合体；结合，联合；
```

commemorate	v.纪念，用以纪念

compensate	v.赔偿，偿付；弥补，补偿

```
-pense/-pens (penny 支付)
```

compete

competent	adj.能干的，能胜任的；合格的，过得去的；

competitive	adj.竞争的；好竞争的，好胜的；有竞争力的

comprehend	v.理解，领悟；包括，包含

```
-prehend (抓住)
comprehension	n.理解力，领悟力；
comprehensive	adj.综合性的，全面的；有理解力的 n.综合中学；专业综合测验
```

conclude	v.断定，推断出；结束，终止；达成，做成，完成

```
-clud (关闭)
include	v.包含；把……列为一部分，把……算入
exclude	vt.（故意）不包括，把……排除在外；认为……不可能，排除……的可能；阻挡；开除（学生）；防止……的发生
```

concentrate	v.集中注意力，聚精会神；使集中，聚集；浓缩

```
con- + -center + -ate
```

confirm	v.确定，确认；证实，证明；批准，认可；使坚定，加强；

```
firm	adj.结实的，坚硬的；牢固的，稳固的；
consolidate	v.使巩固，使加强；合并，统一
solid	adj.坚硬的；固体的，固态的；
```

constrain	v.限制，约束；强迫，迫使

conflict	n.争执，争论，分歧；战斗，战争；抵触，矛盾

​				v.冲突，抵触



### pre-

preschool	*n.*幼儿园，育幼院

prehistory	*n.*史前

precise	*adj.*精确的，准确的；恰好的，确切的；



## -voc-/-vok- = voice,call

voice	v.表达...观点，吐露，n.声音

```
convey	v.传送，表达
express one`s opinion 抒发己见
with one voice 异口同声
```

vocal	*adj.*嗓音的，直言不讳的

```
-al 形容词后缀
```

vocation	n.职业

```
vacation	假期
vocational school	n.职业学校
```

provoke	v.激怒，引起，激励

```
anger = indignation = rage = enrage = outrage
anger	n.愤怒，怒火
indignation	n.义愤
rage	n.盛怒，狂怒
enrage	v.激怒，使暴怒
outrage	n&v.愤慨，使愤慨
```

revoke	*v.*撤回，撤销	re- 向后

advocate	*v.*拥护，提倡	ad- 方向

evoke	*v.*引起，唤起 =recall   e-前缀->向外的



### a+词根首字母双写表“加强”(ab-/ac-等)

appoint	v.任命，指认

accelerate	*v.*（使）加快，促进	-celer- 速度

```
decelerate	减速
```

accumulate	v.积累	-cumul- 堆积

account	n.账户，账单，客户，v.认为，说明

```
sb account that... 某人认为
on account of	由于
by all account	众所周知
leave sth out of account	不考虑...
account for  对...负责
			导致...
			对...做出解释
			占比  = take up  =  make up  = occupy
accountant	n.会计	-ant 名词后缀 ...的人
discount	n&v打折
```

aggravate	v.使扩大，使加重	-gra- great

assent	n&v.同意，批准

```
consent	n&v.许可，允许
```

### ex-/e- 表示出去

exit	*n.*出口，通道	*v.*退出（电脑程序）；退场

exclude	*vt.*（故意）不包括，把……排除在外；

### ex- 表“前面的，前任的”

ex-wife	*n.*前妻

ex-president	*n.*前任总统

## claim/clam = cry out,shout 表“呼喊，叫喊”

claim	*v.*声称，断言，[生]索取，要求

```
disclaim	v.否认	dis- ↓
acclaim		v&n.称赞，赞扬	ac-表加强
exclaim		v.呼喊，惊叫	ex- 向外
proclaim	v.宣告，声明
proclamation	n.正式的公告，宣言；宣布，公布
reclaim		v.要求归还，回收，开拓
reclamation		n.开垦；收回；再利用；矫正
```

## nounce/nunci = speak

### announce

announce	v.宣告，播报

announcement	n.公告，布告

announcer	n.广播员，宣告者

### pronounce

pronounce	v.发音，宣判，宣告

pronouncement	n.声明，宣告

pronunciation	n.发音

pronounced	a.明显的，显著的

```
obvious	a.显然的，明显的
noteworthy	a.值得注意的，显著的
noticeable	adj.显而易见的，明显的；值得注意的
```

### denounce

denounce	v.指责，抨击

```
condemn	v.谴责，严厉指责
criticize	v.批评，指责；评论
complain	v.抱怨，投诉
```

### de- 表↓ ，“去掉，变坏，离开，变慢，向下”等负面词

departure	*n.*背离；离开，启程

```
department	n.部门
```

deforest	*v.*砍伐树林

devalue	*vt.*使贬值；降低……的价值

depress	v.沮丧，萧条

decode	v.解码

```
code	[生]准则，规范
```

### de- 表↑，“加强，使...成为”等正向词

depict	v.描述，描绘	-pict = picture

define	v.定义，解释，[生]使明确

```
-fin- 结束
infinite	a.无限的，无穷的
finite	a.有限的，限定的
fine	[生]罚金
```

## dict/dic = say,assert 表“说话，断言”

### dictate

dictate	n&v.命令，规定；口述，听写

dictator	*n.*独裁者；专横的人

dictatorship	*n.*专政；独裁权；独裁者职位

```
-ship	抽象名词后缀，表示某种关系，状态，技能
readership	n.读者
membership	n.会员
friendship	n.友谊，朋友关系
hardship	n.苦难
ownership	n.所有权
```

dictionary	n.字典

### predict

predict	v.预言，预计

```
forecast	v.预测，预报
foresee		v.预见
foretell	v.预言，预测
expect		v.期待，期望
unpredictable	a.不可预测的
anticipate	v.预期，预料
	participant		n.参与者
	participate		v.参加
```

prediction	n.

### dedicate

dedicate	v.致力于，献身于

```
be dedicate to = be devoted to 从事于，致力于
```

dedication	n.

### indicate

indicate	v.指明，表明

indicator	n.指示器

indicative	a.指示的，n.陈述语气

compass	n.罗盘

### addict

addict	v.使上瘾，使沉迷，n.上瘾的人

```
be addicted to 对...上瘾
be hooked on 迷上
	hook	n.钩子
be indulged into 沉迷于
	indulged	v.沉溺，沉浸
```

addiction	n.瘾，入迷

### verdict

verdict	*n.*裁定，评判，意见	ver- = true(真实)

### indict

indict	v.控告

```
accuse	v.控告，指责
accusation	n.控告，指责
lecture	n.教训
```

indictment	n.

### contradict

contradict	v.反驳，否认，相矛盾

contradiction	n.矛盾

contradictory	a.相互矛盾的，对立的，n.矛盾命题

```
A in contradiction to/with B	A与B不同
by contrast = on the contrary	与之相反的是
compare	v.比较
comparison	n.比较

justify
justify	+ sb	为...辩护
justify + sth	证明...合理
justice	n.公平，正义，[生]法官
jury	n.陪审团
judge	n.法官
```

### contra- 表示相反

contrary	a.相反的

contradict

contradiction

contradictory

## lect/leg = speak,read

dialect	n.方言，土话

```
linguist	n.语言学家
```

legible	a.清晰的，易读的

```
illegible	a.难以分辨的
```

legend	n.传说，传奇人物

```
legendary	a.传说的，非常有名的
```

elect	v.选举，选择

```
election	n.选举，当选
eligible	a.符合条件的，合适的，n.适任者
```

select	v.挑选

```
selection	n.选择，挑选
```

neglect	v.疏忽，忽视，n.忽略，忽视	= ignore，overlook

## log/logue = speak

dialogue	n.对话

analogy	n.类比，比拟

```
be analogous to	与...类比，与...相似
the same as
be similar to
be parallel to
四个都可表示 和...相似
```

apology	n.道歉

```
apologize	v.道歉，谢罪
apologize for doing sth
make an apologize to sb
```

catalogue	n.目录，一系列

prologue	n.开场白，v.以...作为前言/序

eloquent	a.雄辩的，有说服力的

